...
DISPLAY_TX_TO_ESP_RX = 18
DISPLAY_RX_TO_ESP_TX = 17
display_uart = UART(
    1,
    baudrate=115200,
    tx=DISPLAY_RX_TO_ESP_TX,
    rx=DISPLAY_TX_TO_ESP_RX
)
...
print("Starting up (filter reset + taring) took {} ms".format(elapsed_time))
# Loop equivalent
while True:
    ...
    if (instruction == "1"):
        ...
        scale.set_scale_factor(calibration_weight)
        end_time = ticks_ms()
        print("Calibration took {} ms".format(ticks_diff(end_time, start_time)))
    elif (instruction == "2"):
        start_time = ticks_ms()
        print("Current offset is set at", scale.get_offset())
        scale.set_offset()
        end_time = ticks_ms()
        print("Taring only took {} ms".format(ticks_diff(end_time, start_time))) 
    else: 
