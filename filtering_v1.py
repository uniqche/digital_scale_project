...
class HX711:
    def __init__(self, dout_pin, sck_pin, gain=128):
        ...
        self.scale_factor = 1414.5 # 1415 too high, 3kg is slightly low by about 1.0–1.3 g
        # 1414.5 for 3kg
    ...
    def set_scale_factor(self, calibration_weight):
        ...
        print("Previous scale factor:", self.scale_factor)
# ***********************
        ...
        self.scale_factor = truncated_scale_factor
# ***********************
        print("Untruncated scale factor:", untruncated_scale_factor)
        print("Truncated and stored scale factor:", self.scale_factor)
        # print("Calibration complete. New factor = {:.4f}".format(self.scale_factor))
...
while True:
    ...
    if (instruction == "1"):
        ...
        while (True):
            ...
            if len(scale.moving_average_buffer) >= moving_average_filter_size:
                # print("measured weight: {:.1f} g\tknown_weight: {}\tadc: {}\toffset: {}\tscale_factor: {}\tcal_weight:{}".format(displayed_weight, known_weight, median_filtered_raw, scale.get_offset(), scale.get_scale_factor(), scale.get_cal_weight()))
                print("displayed weight: {} g\tcalulated weight: {}\tadc: {}\toffset: {}\tscale_factor: {}\tcal_weight:{}".format(displayed_weight, weight, median_filtered_raw, scale.get_offset(), scale.get_scale_factor(), scale.get_cal_weight()))
                # count += 1
                if (count == 20):
                    break